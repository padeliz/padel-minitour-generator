        ...
        if ($lotteryRule->rule_prize_quantity > LotteryLucky::countWhere('lottery_rule_id = ? AND accepted_at IS NOT NULL', [$lotteryRule->id()])) {
            ...
            if (!$luckyOne) {
                ...
                $editionDivisionsEndingTimes = DB::column(
                    ...
                    "playing_end_time",
                    "edition_id = ? AND (playing_end_time > ? OR playing_start_time = ?)",
                    [$form->value('id_edition'), $current_time, $drawn_participation->playing_start_time]
                );

                if ($editionDivisionsEndingTimes) {
                    $adjustedNow = max( // in case you run this script before the edition started
                        strtotime($edition->date . " " . $drawn_participation->playing_start_time . " +20 minutes"),
                        time()
                    );
                    $soonestDivisionFinish = min(
                        array_filter($editionDivisionsEndingTimes, fn($v) => $v >= $drawn_participation->playing_start_time)
                    );
                    $intervalLotteryEnding = max(
                        time(), // in case edition almost finished
                        strtotime($soonestDivisionFinish . " -30 minutes") // some buffer at the end
                    );
                    $intervalLotterySecondsLeft = ($intervalLotteryEnding - $adjustedNow);

                    $luckyOne->available_at = $adjustedNow + max(15, round($intervalLotterySecondsLeft / $remaining_draws_nr));
                } else {
                    // all divisions already finished

                    $luckyOne->available_at = strtotime("now +10 seconds");
                }
