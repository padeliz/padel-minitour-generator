...
/**
 * Read-only metrics over the in-use fixed-teams templates.
 */
final class MetricsTemplatesFixedTeamsCommand extends Command
{
    use MetricsFormatterTrait;

    protected static $defaultName = 'templates:metrics-fixed-teams';
    ...
    {
        $this
            ->setDescription('Read-only metrics over committed fixed-teams templates.')
            ->setHelp(implode("\n", [
                ...
                'matching JSON files on disk (defaults: repeat=1, courts=1, fixed-teams=yes).',
            ...
            ->addOption('fixed-teams', null, InputOption::VALUE_REQUIRED, 'Filter by fixed teams (0 or 1; default 1)')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $combos = $this->resolveMetricsCombos($input, $this->repository, $version, $combinations, true);
        } catch (\InvalidArgumentException $e) {
            $io->error($e->getMessage());
        ...
        if ($missing > 0) {
            ...
            $output->writeln('Run <comment>php bin/console templates:regenerate --templates-version=N --fixed-teams=1</comment> to (re)generate them.');
        }
